	...
	:m_width(width), m_height(height), m_current(nullptr), m_next(nullptr), m_specials(specials)
{
	...
	//checks arbitrary minimum hieght and width of 4
	if(width < 4 || height < 4 || 
		width > MAX_WELL_WIDTH - 5 || height > MAX_WELL_HEIGHT - 2)
    {
        exit(1);
    }
}
...
void Well::displayInsides(Screen& screen, int x, int y) const
{
	for(int i = 0; i < m_height; i++)
		//going down each columns
	{
...
bool Well::displayNext(Screen& screen, int x, int y) const
{
	if(m_current == nullptr)
		//only happens in beginning
	{
		return false;
	}

	//extracts the oreintation of m_next
	std::string str = m_next->feed();

	//clears previous display and replaces it with new piece
	for(int i = 0; i < 4; i++)
	{
...
bool Well::drop()
{
	...
	//check if able to "drop" (display) piece. If not, game over
	for(int i = 0; i < 4; i++)
	{
		for(int j = 0; j < 4; j++)
		{
...
int Well::go(char key)
{
	if(m_current->getID() == 9)
		//crazy piece. just switch around inputs
	{
		switch(key)
		{
			case '6':
	...
	std::string next = m_current->feedNextOrientation(); //only used for rotating
	switch(key)
	{
		case '8': //rotates
		case 'w':
...
bool Well::check(std::string str, int x, int y) const
{
	for(int i = 0; i < 4; i++)
	{
		for(int j = 0; j < 4; j++)
...
void Well::foam(int x, int y)
{
	if(x > m_x + 3 || x < m_x - 1 || y > m_y + 3 || y < m_y - 1)
		//makes sure it doesn't affect anything outside 5 * 5 area
	{
		return;
	}
	else
...
int Well::clearRows()
{
	...
	for(int i = 0; i < m_height; i++)
	{
		...
		{
			if(m_things[i][j] == ' ')
				//if space, current row can't be full
			{
				clean = false;
				break;
			}
